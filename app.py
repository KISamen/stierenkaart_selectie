...
# Gebruik een brede layout
st.set_page_config(layout="wide")

# Zorg dat we in de session state een stap-variabele hebben
if "step" not in st.session_state:
    st.session_state.step = 1

# Functie om een Excelbestand in te lezen en de kolomnamen te trimmen
def load_excel(file):
    try:
...
# Functie om de 4 bestanden te verwerken en de stierenkaart te genereren
def process_files(uploaded_crv, uploaded_pim, uploaded_prijslijst, uploaded_joop, debug_mode=False):
    # Lees bestanden
    ...
    # Maak tijdelijke merge-sleutel in alle bestanden
    ...
    # Definieer de mappingtabel (Let op: we gebruiken "KI_Code" als bron en willen dat de output "KI-code" heet)
    mapping_table = [
